...
public class CartDaoMem implements CartDao {
    ...
    private Map<Product, Integer> newShopcart;
    ...
    public List<Cart> findActive(int user_id) {
        ...
        try {
            ...
            while (rs.next()){
                ...
                newShopcart = new HashMap<>();
                newShopcart = mapConverter(shopcart);
                Cart newCart = new Cart(rs.getInt("id"), rs.getInt("user_id"), rs.getString("active"), rs.getString("historydate"), newShopcart, ad.getAddress(rs.getInt("address_id")), rs.getInt("totalprice"));
                data.add(newCart);
            }
            con.close();
        } catch (Exception ex) {
    ...
    @Override
    public List<Cart> getAllHistory(int user_id) {
        data = new ArrayList<>();
        try {
            Class.forName("org.postgresql.Driver");
            Connection con = DriverManager.getConnection("jdbc:postgresql://localhost:5432/shop", "postgres", "Madrid1975");
